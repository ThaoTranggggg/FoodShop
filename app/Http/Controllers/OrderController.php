...
class OrderController extends Controller
{
    ...
    {
        $orders = OrderModel::where('status', 'wait')->get();

        // Lấy tổng tiền tất cả sản phẩm trong giỏ hàng
    ...
    public function API_apply_discount_to_cart(Request $request)
    {
        ...
        if (strtotime($discount->start_at) > time()) {
            return $this->res(false, 'Mã giảm giá chưa tới thời gian áp dụng');
        }
    ...
    public function API_get_order_detail(Request $request)
    {
        ...
        // Tính tổng tiền nếu có mã giảm giá
        foreach ($products as $product) {
